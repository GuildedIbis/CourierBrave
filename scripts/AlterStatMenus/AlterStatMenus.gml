...
var _health = "Increases max health (NEXT: " + string(obj_player.max_hp + 3 + round(obj_player.vitality/15)) + ")";
...
if (point_in_rectangle(_mouseX,_mouseY,140,106,180,119))
{
	...
	if (mouse_check_button_pressed(mb_left)) and (obj_player.beans >= _cost)
	{
		...
		obj_player.max_hp = obj_player.max_hp + 3 + round(obj_player.vitality/15);
		obj_player.hp = obj_player.hp + 3 + round(obj_player.vitality/15);
		//Reset Buy/Sell Menu
	}
...
var _timer = "Increases max energy (NEXT: " + string(50 + ((obj_player.energy + 1) * 3)) + ")";
draw_set_color(c_black);
...
var _grace = "Applies to magic skills.";
draw_set_color(c_black);
