...
if (obj_inventory.quest_grid[# 8, 0] = false)
{
	...
	draw_sprite(spr_npc_mesa36,0,258+6,136+6);
...
if (obj_inventory.quest_grid[# 8, 0] = true) and (obj_inventory.quest_grid[# 8, 1] <= 5)
{
	draw_set_font(fnt_text);
	draw_set_halign(fa_left)
	draw_set_valign(fa_top)
...
draw_sprite(spr_npc_mesa36,0,258+6,136+6); //Portrait
...
draw_text_transformed(114,149,"YAKFLOWER PATH",.5,.5,0);
...
//
//Mesa Yakflower Text
function MesaYakflowerText(){

//Finish Introduction
if (obj_inventory.quest_grid[# 8, 1] < 5)
{
	draw_set_font(fnt_text);
	draw_set_halign(fa_left)
	draw_set_valign(fa_top)
	...
	{
		speaker = 1;
		text_string = "They call this region of pink grasses Yakflower Path."
		_SubString = string_copy(text_string,1,letter_counter);
		draw_text_transformed(72,128,"Press E to Continue",.5,.5,0);
	}
	if (string_counter = 1)
	{
		speaker = 1;
		text_string = "There are three totems at the far end of the glade.\nThey seemed to respond to my precense,\nbut to what end I'm unsure."
		_SubString = string_copy(text_string,1,letter_counter);
		draw_text_transformed(72,128,"Press E to Continue",.5,.5,0);
	}
	if (string_counter = 2)
	{
		speaker = 1;
		text_string = "Do watch for the Moth's though.\nThey are quite aggressive...\nand I fear there is worse in the depths."
		_SubString = string_copy(text_string,1,letter_counter);
		draw_text_transformed(72,128,"Press E to Continue",.5,.5,0);
	}
...
//
//
//Mesa Yakflower Restart 
function MesaRestartMenu(){
...
{
	speaker = 1;
	text_string = "Restart Lair: Yakflower Path?."
	_SubString = string_copy(text_string,1,letter_counter);
	...
	if (point_in_rectangle(_mouseX,_mouseY,70,159,158,174))//Select Yes: Restart Quest and Close Menu
	{
		...
		if (mouse_check_button_pressed(mb_left))
		{
			obj_inventory.yakflower_lair[0] = 0;
			obj_inventory.yakflower_lair[1] = 0;
			obj_inventory.yakflower_lair[2] = 0;
			obj_inventory.yakflower_lair[3] = 0;
			obj_inventory.yakflower_lair[4] = 0;
