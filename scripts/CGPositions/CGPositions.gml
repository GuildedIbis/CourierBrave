...
draw_sprite_ext(spr_card_all,card_id,x,y,1,1,0,c_white,1.0);
...
switch(card_place)
{
	case 0: //Hand
		...
		if (_handSize < 11)
		{
			var _leftAlign = 225 - (_handSize * 15);
			//var _space = 64;
			var _cardX = _leftAlign + (30 * card_position);
		}
		else
		{
			var _leftAlign = 225 - (11 * 15);
			//var _space = max(1,26 - ((_handSize - 11)));
			var _space = max(1,250/_handSize); //It's 280 from one edge to the other, -30 for the width of the card
		...
		//Select
		if (point_in_rectangle(_mouseX,_mouseY,_cardX,_cardY,_cardX+30,_cardY+42))
		{
			draw_sprite_ext(spr_card_all,card_id,419,13,1,1,0,c_white,1);
			draw_sprite_stretched(spr_highlight_nineslice,0,_cardX-1,_cardY-1,32,44);
