...
//
//
//Gorog Captain Create
function GorogCaptainCreate(){
name = "Gorog Captain"
boss = true;
home_state = GorogCaptainFree;
entity_step = home_state;
entity_drop = GorogCaptainDrop;
...
//
//
//Gorog Captain Free State
function GorogCaptainFree(){
		...
		{	
			...
			if (timer1 > 0) and (timer2 > 0) //AI feature: only use shield as last option
			{
				if (timer3 <= 0)//Shield Timer
				{
					...
					timer3 = 180;
					inv_dur_timer = 180;
					entity_step = GorogCaptainShield;
				}
			}
			if (point_in_circle(obj_player.x,obj_player.y,x,y,16))
			{
				...
				{
					entity_step = GorogCaptainHeavySlashA;
				}		
		...
		else
		{
			...
			{	
				...
				if (_rATK = 0)
				{
					path_end();
					timer2 = 15;
					entity_step = GorogCaptainKnifeThrow;
				}
				else
				{
					path_end();
					timer2 = 24;
					entity_step = GorogCaptainChainHookToss;
				}
			}
		}
...
//
//
//Gorog Captain Shield
function GorogCaptainShield(){
if (obj_game.gamePaused = false)
{
	if (timer1 > 0) timer1 = timer1 - 1;
	if (timer2 > 0) timer2 = timer2 - 1;
...
//
//
//Gorog Captain Slash State
function GorogCaptainHeavySlashA(){
if (obj_game.gamePaused = false)
{
	if (timer2 > 0) timer2 = timer2 - 1;
	if (timer3 > 0) timer3 = timer3 - 1;
	...
	if (animation_end)
	{
		...
		{
			entity_step = GorogCaptainHeavySlashB;
		}
		else 
		{
			timer1 = 30;
			sprite_index = enemy_idle;
			entity_step = GorogCaptainFree;
		}
		animation_end = false;
	}
}
...
//
//
//Gorog Captain Heavy Slash B
function GorogCaptainHeavySlashB(){
if (obj_game.gamePaused = false)
{
	if (timer2 > 0) timer2 = timer2 - 1;
	if (timer3 > 0) timer3 = timer3 - 1;
	...
	if (animation_end)
	{
		...
		entity_step = GorogCaptainReposition;
		animation_end = false;
	}
}
}
...
//
//
//Gorog Captain Chain Hook Toss
function GorogCaptainChainHookToss(){
if (obj_game.gamePaused = false)
{
	if (timer1 > 0) timer1 = timer1 - 1;
	if (timer2 > 0) timer2 = timer2 - 1;
	...
	{	
		attack_counter = 0;
		entity_step = GorogCaptainChainHookWait;
		animation_end = false;
...
//
//
//Gorog Captain Chain Hook Wait
function GorogCaptainChainHookWait(){
if (obj_game.gamePaused = false)
{
	if (timer1 > 0) timer1 = timer1 - 1;
	if (timer2 > 0) timer2 = timer2 - 1;
...
//
//
//Gorog Captain Knife Throw
function GorogCaptainKnifeThrow(){
if (obj_game.gamePaused = false)
{
	if (timer1 > 0) timer1 = timer1 - 1;
	if (timer2 > 0) timer2 = timer2 - 1;
...
//
//
//Gorog Captain Reposition
function GorogCaptainReposition(){
//Timer
