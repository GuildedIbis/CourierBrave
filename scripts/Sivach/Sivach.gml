...
//
//Sivach Set (create)
function scr_player_sivach_set(){
//
form = 7;
form_type = 1;
home_state = scr_player_sivach_set;
free_state = scr_player_sivach_free;
state_script = scr_player_sivach_free;
magicP_script = scr_player_sivach_helixStone;
magicA_script = scr_player_sivach_helixStone;
magic_primary = true;
idle_sprite = spr_player_sivach_idle;
roll_sprite = spr_player_sivach_roll;
crystal_sprite = spr_player_sivach_crystal;
//recharge_sprite = spr_player_sivach_recharge;
arm_sprite = spr_player_sivach_castArm;
obj_cursor.curs_script = scr_cursor_sivach;
//
weapon_count = -1;
casting = false;
max_weapon_count = -1;
...
//
//
//Sivach Free (home) state
function scr_player_sivach_free(){
//Set
walk_spd = 1.25;
attacking = false;
casting = false;
...
{
	direction = input_dir;
	sprite_index = spr_player_sivach_run;
}
else sprite_index = spr_player_sivach_idle;
...
if (key_attackW)
{
	if (thundux = false)// and (stamina >= 20)
	{
		...
		//stamina = stamina - 20;
		attack_script = scr_player_sivach_regalBlade;
...
if (key_attackS) and (yellow_special >= 100)
{
	if (watervice = false)
	{
		...
		attack_script = scr_player_sivach_goldArcs;
		state_script = scr_player_attack;
	}
}
...
//
//Sivach Melee State
function scr_player_sivach_melee(){
//Set
attacking = true;
...
//Attack Start
if (sprite_index != spr_player_sivach_melee)
{
	//Start Animation From Beginning
	//var _atkSpeed = round(15 * (1 + (obj_inventory.form_grid[# 0, 5]/10)));
	sprite_index = spr_player_sivach_melee;
...
//scr_player_attack_calculate_weapon(spr_player_sivach_slash_hitbox,obj_player,1.5,-1,-1,-1,-1,-1,5);
...
//
//Sivach Magic State
function scr_player_sivach_helixStone(){
//Set
walk_spd = 1.0;
attacking = true;
casting = true;
...
{
	direction = input_dir;
	sprite_index = spr_player_sivach_runCast;
}
else sprite_index = spr_player_sivach_cast;
if (_oldSprite != sprite_index) local_frame = 0;
...
{	
	...
	with (instance_create_layer(ldX + dir_offX, ldY + dir_offY,"Instances",obj_projectile))
	{
		...
		timer1 = 50;
		projectile_sprite = spr_projectile_sivach_helixStone;
		projectile_script = scr_projectile_helixStone;
		idle_sprite = spr_projectile_sivach_helixStone;
		hit_by_attack = -1;
...
//
//
//Sivach Helix Stone Projectile Script
...
//
//
//Sivach Special State
function scr_player_sivach_special(){
//Set
attacking = true;
...
//Initiate Attack
if (sprite_index != spr_player_sivach_cast_special)
{
	//Start Animation From Beginning
	sprite_index = spr_player_sivach_cast_special;
	//sprite_set_speed(sprite_index,15,spritespeed_framespersecond);
	local_frame = 0;
	image_index = 0;
	//Clear Hit List
...
if (animation_end)
{
	...
	with (instance_create_layer(x,y-6,"Instances",obj_projectile))
	{
		...
		idle_sprite = spr_projectile_sivach_heatBomb;
		projectile_sprite = spr_projectile_sivach_heatBomb;
...
//
//
//Sivach Gold Arcs Projectile Script
