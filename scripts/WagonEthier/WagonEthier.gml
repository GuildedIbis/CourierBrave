...
sprite_index = spr_npc_wagonCat_idle_sit;
npc_idle = spr_npc_wagonCat_idle_sit;
npc_move = spr_npc_wagonCat_idle_stand;
sound = snd_npc_mouse;
interact = 20;
npc_spd = 1.0;
path = -1;
	...
	if (letter_counter >= string_length(text_string))
	{
		...
		if (point_in_rectangle(_mouseX,_mouseY,70,92,250,112))
		{
			...
			if (mouse_check_button_pressed(mb_left))
			{
				...
				_SubString = string_copy(text_string,1,letter_counter);
				e_page = true;
				scr_game_room_change(416,232,rm_beetrap_beetraCity_04,4,5,obj_inventory.beetraCity_map_ary,obj_game.beetraCity_enemy_grid,false,-1)
			}
		}
