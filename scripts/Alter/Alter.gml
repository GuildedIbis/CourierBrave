...
var _mouseX = device_mouse_x_to_gui(0);
var _mouseY = device_mouse_y_to_gui(0);
var _recipeMenu = array_create(18);
...
if (obj_player.crull_ary[obj_player.crull_selected] = -1)
{
	for (var i = 0; i < 5; i = i + 1)
	{
		draw_sprite_stretched(spr_menu_circle16,1,49,53+(17 * i),100,16);
		if (obj_inventory.crull_recipe[i] = true)
		{
			draw_sprite_ext(spr_menu_crull_recipe,i,52,56,1,1,0,c_white,1.0);
			if (point_in_rectangle(_mouseX,_mouseY,49,53+(17*i),149,69+(17*i)))
			{
				draw_sprite_stretched(spr_highlight_circle,0,48,52+(17*i),102,18);
				text_gui = _recipeMenu[i]
			}
		}
	}
}
...
draw_sprite_stretched(spr_rog_all,0,234,117,16,16);
draw_text_transformed(232,128,"1\n("+ string(obj_inventory.rog_array[0])+")",.6,.6,0);
draw_sprite_stretched(spr_rog_all,1,258,117,16,16);
draw_text_transformed(262,128,"1\n("+ string(obj_inventory.rog_array[1])+")",.6,.6,0);
